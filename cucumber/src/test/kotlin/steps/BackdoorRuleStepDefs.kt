...
class BackdoorRuleStepDefs {
    ...
    fun backdoorAddComment(caseName: String, comment: String, conditions: DataTable) {
        restClient().buildRule(caseName, Addition(comment), parseConditionsUsingIsSyntaxOnly(conditions))
    }
    ...
    fun backdoorRemoveComment(caseName: String, comment: String, conditions: DataTable) {
        restClient().buildRule(caseName, Removal(comment), parseConditionsUsingIsSyntaxOnly(conditions))
    }
    ...
    fun backdoorReplaceComment(caseName: String, toGo: String, replacement: String, conditions: DataTable) {
        restClient().buildRule(caseName, Replacement(toGo, replacement), parseConditionsUsingIsSyntaxOnly(conditions))
    }
}

fun parseConditionsUsingIsSyntaxOnly(dataTable: DataTable): List<ConditionDescriptor> {
    return dataTable.asList().map { conditionText ->
        val parts = conditionText.split(" is ", limit = 2)
        require(parts.size == 2) { "Condition must be in the form 'attribute is \"value\"': $conditionText" }
        ConditionDescriptor(parts[0].trim(), parts[1].trim())
    }
}
