Feature: Phase 1 â€” Suggested conditions are ranked by the rule action, the cornerstones, and historical conditions, and capped at a usable size.
  ...
  #   Set A â€” Action targeting
  #   Set B â€” Cornerstone discrimination
  #   Set C â€” Historical conditions.
  #
  ...
  Scenario: A condition that discriminates between the session case and the cornerstone is preferred
    # Waves â‰¤ 1.5 holds only for Bondi (discrimination = 1).
    # Waves â‰¥ 1.5 holds for both (discrimination = 0).
    # Sun is "hot" holds for both (discrimination = 0).
    # UV index â‰¤ 1 and UV index â‰¥ 1 hold for both (discrimination = 0).
    ...
    Then the suggested condition "Waves â‰¤ 1.5" should appear before all of the following suggestions:
      | Waves â‰¥ 1.5             |
      | Sun is "hot"              |
      | UV index â‰¤ 1.0          |
      | UV index â‰¥ 1.0          |
  ...
  # condition "eGFR â‰¥ 70" across multiple cases. When the user later adds
  # the SAME comment to Einstein, "eGFR â‰¥ 70" surfaces high purely on
  # historical signal â€” even though it has no comment-text overlap and no
  ...
  Scenario: Conditions historically used for the same comment rank above unrelated suggestions
    # One historical rule for "Elevated haemoglobin may be significant". It uses "eGFR â‰¥ 70".
    ...
    Then the suggested condition "eGFR â‰¥ 70" should appear before all of the following suggestions:
      | AST increasing         |
  ...
  Scenario: Conditions used by multiple historical rules for the same comment rank above conditions used only once
    ...
    Then the suggested condition "Waves â‰¥ 1.0" should appear before the following suggestion:
      | Sun is "hot |
  ...
  Scenario: Historical signal applies only to rules whose conclusion matches the action's target conclusion
    ...
    Then the suggestion "eGFR â‰¥ 70" should NOT appear
