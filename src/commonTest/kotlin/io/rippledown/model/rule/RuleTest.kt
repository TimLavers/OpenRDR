...
internal class RuleTest : RuleTestBase() {
    ...
    fun should_not_be_structurally_equal_if_different_conditions() {
        val rule1 = Rule(1,null, conclusion1, setOf(cond("a")))
    ...
    fun single_condition_which_is_true_for_case() {
        val rule = Rule(1, null, conclusion1, setOf(cond("vark")))
    ...
    private fun setupRuleWithOneChild(): Rule {
        val rule = Rule(100, null, conclusion1, setOf(cond("a")))
        val childRule = Rule(200, null, conclusion2, setOf(cond("b")))
        rule.addChild(childRule)
        return rule
    }
