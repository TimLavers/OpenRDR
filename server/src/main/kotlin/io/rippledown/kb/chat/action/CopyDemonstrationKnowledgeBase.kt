...
import io.rippledown.constants.chat.nameForDemonstrationCopyMessage
...
data class CopyDemonstrationKnowledgeBase(val sample: SampleKB, val kbName: String) : KbManagementAction {
    override suspend fun doIt(kbService: KnowledgeBaseService): KbManagementOutcome =
        outcomeForNewKbName(
            kbService, kbName, nameForDemonstrationCopyMessage(sample.title()),
            actionForName = { CopyDemonstrationKnowledgeBase(sample, it) }
        ) { service, name ->
