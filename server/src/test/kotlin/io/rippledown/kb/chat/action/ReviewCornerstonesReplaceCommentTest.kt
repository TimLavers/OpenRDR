...
class ReviewCornerstonesReplaceCommentTest : ActionTestBase() {
    @Test
    fun `should start a rule session to replace a comment`() = runTest {
        //Given
        val action = ReviewCornerstonesReplaceComment(commentToRemove, commentToAdd)
        val ccStatus = CornerstoneStatus(indexOfCornerstoneToReview = 42, numberOfCornerstones = 84)
        coEvery {
            ruleService.startCornerstoneReviewSessionToReplaceComment(any(), commentToRemove, commentToAdd)
        } returns ccStatus
